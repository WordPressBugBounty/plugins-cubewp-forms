...
class CubeWp_Forms_Custom {
	 ...
	 * @var string
	 */
	public static $CubeWp_Forms_version = '1.1.3';
	/**
	...
	public function init() {
		...
		add_action('init', array('CubeWp_Forms_Templates', 'init'), 9);
		add_action('init', array('CubeWp_Forms_Emails', 'init'), 9);
		add_action( 'rest_api_init', array('CubeWp_Forms_Rest_API', 'init' ), 9);
		// Set up localisation.
    ...
    public function Cubewp_Menu($defaut) {
        $settings = array(
			...
			array(
                'id'           =>  'cubewp-custom-form-templates',
                'parent'       =>  'cubewp-form-fields',
                'title'        =>  esc_html__('Templates', 'cubewp-forms'),
                'callback'     =>  'cubewp-custom-form-templates',
            ),
        );
		...
		if ( isset( $cwpOptions['cubewp_forms_mailchimp'] ) && $cwpOptions['cubewp_forms_mailchimp'] == '1' ) {
			$settings[] = array(
				'id'           =>  'cubewp-forms-mailchimp-errors',
                'parent'       =>  'cubewp-form-fields',
                'title'        =>  esc_html__('Mailchimp API Errors', 'cubewp-forms'),
                'callback'     =>  'cubewp-forms-mailchimp-errors'
			);
		}
